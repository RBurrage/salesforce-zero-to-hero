...
export default class MemoryGameLWC extends LightningElement {
    ...
    displayCard(event) {
        ...
        //using concat keeps the array mutable
    ...
    unmatched() {
        this.openedCards[0].classList.add("unmatched", "disabled")
        this.openedCards[1].classList.add("unmatched", "disabled")
        this.action('DISABLE')
        setTimeout(() => {
    ...
    //use renderedCallback whenvever loading an external resource -- lifecycle hook
    renderedCallback() {
        if (this.isLibLoaded) {
            return
        } else {
